...
export function PlanningWorkspace({ mode }: { mode: Mode }) {
  ...
  const [automationAction, setAutomationAction] = useState("Create notification")
  ...
  async function addItem() {
    ...
    if (mode === "automations") result = await supabase.from("automations").insert({ user_id: auth.user.id, name: title.trim(), trigger: "Manual", action: automationAction, enabled: true })
